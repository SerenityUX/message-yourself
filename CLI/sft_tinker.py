...
``tinker_cookbook.supervised.data.conversation_to_datum`` with a model-matched renderer
(GPT-OSS Harmony by default), training the last assistant turn only (matches
``sft_output.json`` rows).

Requires Python **3.11+**, ``TINKER_API_KEY`` in ``.env`` in this directory (or the environment),
and ``pip install -r requirements.txt`` (includes ``tinker`` + ``tinker-cookbook``).

Default base model: ``openai/gpt-oss-120b`` (MoE reasoning model on Tinker). Override with
``--base-model`` / ``--renderer`` if you switch families (e.g. Qwen → ``qwen3_instruct``).

**LR & LoRA rank** — These are the two knobs people mean most often for LoRA, but they are not
the *only* levers: **steps**, **batch size**, **max_length** (seq len), and **seed** also change
fit and cost. **Learning rate** scales update size on the adapter; **rank** scales adapter
capacity. Tinker’s ``hyperparam_utils.get_lr`` does not auto-calibrate GPT-OSS yet, so treat
``--lr`` as manual (default **1.5e-4**; try **1e-4** if unstable on 120B).

Example::

  python3 sft_tinker.py
  python3 sft_tinker.py --steps 300 --base-model openai/gpt-oss-120b
"""
...
# Harmony wire format; user ``system`` rows map to developer (see tinker_cookbook GptOssRenderer).
...
async def run_sft_tinker_async(
    *,
    ...
    if not os.environ.get("TINKER_API_KEY"):
        raise SystemExit(
            "Set TINKER_API_KEY in .env here (see https://tinker-docs.thinkingmachines.ai/tinker/quickstart/)"
        )
    ...
    dump_path = (api_dump_path if api_dump_path is not None else DEFAULT_API_DUMP).expanduser().resolve()
    ...
    if run_smoke_inference and tinker_path:
        ...
        try:
            ...
            params = types.SamplingParams(max_tokens=48, temperature=0.45)
    ...
    meta_path = (metadata_out if metadata_out is not None else DEFAULT_METADATA).expanduser().resolve()
    ...
    *,
    ...
    return asyncio.run(
        run_sft_tinker_async(
            ...
            api_dump_path=api_dump_path or DEFAULT_API_DUMP,
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help="Tinker base model id (default: openai/gpt-oss-120b)",
    )
    p.add_argument(
        "--renderer",
        default=DEFAULT_RENDERER,
        help="tinker_cookbook renderer (default: gpt_oss_no_sysprompt for gpt-oss-120b)",
    )
    p.add_argument(
        "--lora-rank",
        type=int,
        default=DEFAULT_LORA_RANK,
        help="LoRA rank on Tinker (must be a power of 2, e.g. 8, 16, 32; lower if overfitting)",
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=DEFAULT_LR,
        help="Adam LR on Tinker (default 1.5e-4; try 1e-4 if unstable, 2e-4 if underfitting)",
    )
    ...
    p.add_argument(
        ...
        default=None,
        metavar="DIR",
        help="Download LoRA to this dir (default: ./tinker_lora_sft_adapter)",
    )
    p.add_argument(
        "--skip-indefinite-ttl",
        action="store_true",
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Write sft_tinker_metadata.json here (for parallel experiment runs; default: ./sft_tinker_metadata.json)",
    )
    p.add_argument(
        "--api-dump-out",
        type=Path,
        default=None,
        help="Write API capture JSON here (default: ./sft_tinker_api_dump.json or alongside --metadata-out)",
    )
    args = p.parse_args()
    ...
    meta_out = args.metadata_out.expanduser().resolve() if args.metadata_out else None
    dump_out = args.api_dump_out.expanduser().resolve() if args.api_dump_out else None
    if dump_out is None and meta_out is not None:
        dump_out = meta_out.parent / "sft_tinker_api_dump.json"
