...
Legacy 2-turn ``[user, assistant]`` rows get the same system prompt prepended at load time.
...
def _normalize_sft_messages(msgs: Any) -> list[dict[str, str]] | None:
    """Return [system, user, assistant] content strings for training."""
    ...
    if (
        len(clean) >= 3
        and clean[0]["role"] == "system"
        and clean[1]["role"] == "user"
        and clean[2]["role"] == "assistant"
    ):
        return clean[:3]
    return None
...
class SFTChatDataset(Dataset):
    """One JSON example [system, user, assistant] → mask through user; train assistant only."""
